...
Page({
  ...
  onLoad: function (options) {
    ...
    var selectParentId = options.selectParentId;
    this.data.hotSellingZone = options.hotSellingZone;
    if (this.data.hotSellingZone == true) {
      this.getGoods()
    } else if (selectParentId.length > 0) {
      var category = AV.Object.createWithoutData('Category', selectParentId);
      this.getSubCategorys(category);
    } else {
      // 生成Category对象
      var category = AV.Object.createWithoutData('Category', categoryId);
      this.data.subCategorieIds.push(category)
   ...
   * 获取全部分类
   */
  getSubCategorys: function (parent) {
    var that = this;
    ...
    query.find().then(function (categories) {
      if (parent) {
        ...
        for (var i = 0; i < categories.length; i++) {
          var category = AV.Object.createWithoutData('Category', categories[i].id);
          subIds.push(category);
        }
        that.setData({
          subCategorieIds: subIds
        });
        that.getGoods()
      }
  ...
  getGoods: function (category) {
    ...
    if (this.data.hotSellingZone == 1) {
      query.equalTo("isHot", true);
    } else {
      query.containedIn("category", this.data.subCategorieIds);
    }
